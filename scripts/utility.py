...
# Last-modified: 24 Mar 2023 01:20:18 PM
...
class Region(BED):
    ...
    def __get_mean_signal(self, bw):
        ...
        array = np.array([])
        for bed in self.center:
            signal = bw.get_as_array(bed.chrom.encode('utf-8'), bed.start, bed.stop)
    ...
    def __get_loop_count(self, pairix_dbi):
        ...
        count = 0
        querystr='{}:{}-{}|*'.format(self.chrom, self.start, self.stop)
...
def merge_segment(filename):
    ...
    for line in fin:
        ...
        if tmp_start is None: # first line
            ...
            print("%s\t%d\t%d\t%s" % (tmp_chrom,tmp_start,tmp_stop,tmp_group), file = tmpout)
            tmp_chrom = chrom
            tmp_start = start
            tmp_stop = stop
            tmp_group = group
            ...
            try:
                ...
                print(chrom, tmp_stop, start, file = sys.stderr)
