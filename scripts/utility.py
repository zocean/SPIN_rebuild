...
# Last-modified: 24 Mar 2023 02:02:26 PM
...
class Region(BED):
    def __init__(self, chrom, start, end):
        '''bed or tfbs region'''
        # public
        BED.__init__(self, chrom, start, end)
        self.mid = (self.start+self.end)/2
        self.verbose = 0
        # private
        self.center = [BED(chrom, start, end)] # list of BED object for initial region after we remove regions overlapping with the excluding region
    ...
    def region_init(self, exclude_tabix_list, genome_size):
        ...
        '''
        self.center = subtract_bed(self.chrom, self.start, self.end, exclude_tabix_list)
    def get_anno(self, anno, label, mode, genome_size):
        ...
        if mode == 'coverage':
            ...
            self.anno[lable] = self.__get_loop_count(anno)
    ...
    def __add_label(self, label):
        if label in self.label:
            if self.verbose > 1:
                print("Colname: %s already exists. Update it" % (label), file = sys.stderr)
            else:
    ...
    def __get_coverage(self, tabix_dbi):
        ...
        for bed in self.center:
            try:
                result = tabix_dbi.query(bed.chrom, bed.start, bed.end)
            except:
                continue
            if result is not None:
                for overlap in result:
                    ...
                    o_end = int(overlap[2])
                    for nn in range(max(o_start, bed.start), min(o_end, bed.end)):
                        o_array.append(nn)
    ...
    def __get_mean_signal(self, bw):
        ...
        array = np.array([])
        for bed in self.center:
            signal = bw.get_as_array(bed.chrom.encode('utf-8'), bed.start, bed.end)
    ...
    def __get_anno(self, tabix_dbi):
        ...
        result = tabix_dbi.query(self.chrom, self.start, self.end)
        if result is None:
            return 'NA'
        else:
            o_len = 0
            ...
            for overlap in result:
                ...
                o_end = int(overlap[2])
                name = overlap[3]
                size = min(o_end, self.end) - max(o_start, self.start)
                if size > o_len:
                    o_len = size
                    o_group = name
            if o_group is None:
                o_group = 'NA'
            return o_group
    def __get_loop_count(self, pairix_dbi):
        '''get the number of loop in the region'''
        count = 0
        querystr='{}:{}-{}|*'.format(self.chrom, self.start, self.end)
        result = pairix_dbi.query2D(querystr)
        for overlap in result:
            count += 1
        querystr='*|{}:{}-{}'.format(self.chrom, self.start, self.end)
        result = pairix_dbi.query2D(querystr)
        for overlap in result:
            count += 1
        # remove those form self-loops within queried region
        querystr='{}:{}-{}'.format(self.chrom, self.start, self.end)
        result = pairix_dbi.query(querystr)
        for overlap in result:
            count -= 1
        return str(count)
    def __get_bam_cov(self, bam):
        ...
        cut_count = 0
        for bed in self.center:
            cut_for, cut_rev = bam.get_count(bed.chrom, bed.start, bed.end)
    ...
    def __get_gc(self, genome):
        ...
        for bed in self.center:
            seq = get_seq_from_bed(bed.chrom, bed.start, bed.end, genome)
            for nt in ['A','C','G','T']:
    ...
    def write(self, label_list = None):
        ...
        if label_list is None: # use label list order by default
            return "%s\t%d\t%d\t%d" % (self.chrom, self.start, self.end, (self.start+self.end)/2) + '\t' + '\t'.join(self.anno[label.split(':')[0]] for label in self.label)
        else: # support re-order
            return "%s\t%d\t%d\t%d" % (self.chrom, self.start, self.end, (self.start+self.end)/2) + '\t' + '\t'.join(self.anno[label.split(':')[0]] for label in label_list)
    def header(self,label_list = None):
        if label_list is None:
            return "#chrom\tstart\tend\tmid\t" + '\t'.join(label for label in self.label)
        else:
            return "#chrom\tstart\tend\tmid\t" + '\t'.join(label for label in label_list)
...
class SamHandler(object):
    ...
    def get_count(self, chrom, start, end):
        ...
        if not isinstance(chrom,str) or not isinstance(start, int) or not isinstance(end,int):
            error("chromosomen name (%s) must be string, start (%s) and end (%s) position must be integer" % (chrom,start,end))
            exit(1)
        cut = {'+':[0]*(end-start), '-':[0]*(end-start)}
        for alignread in self.samfile.fetch(str(chrom),start,end):
            ...
            if cut_site < start or cut_site > end -1 :
                continue
            # update cut_table
            if is_rev:
...
class Gene(object):
    ...
    def __init__(self,chrom,start,end,strand,exon_list,cds_list,genename,geneid):
        # public
        self.chrom = chrom
        self.start = start
        self.end = end
        ...
        self.utr3 = None
    def __repr__(self):
        return "%s\t%d\t%d\t%s\t%s" % (self.chrom,self.start,self.end,self.strand,self.name) + '\t' + ','.join(str(item[0])+'-'+str(item[1]) for item in self.exon)
    def getpromoter(self,up,down,genome_size):
        key = "%d_%d" % (up,down)
        if self.strand == '+':
            self.promoter[key] = (max(self.start-up,0), min(self.start-down,genome_size[self.chrom]))
        elif self.strand == '-':
            self.promoter[key] = (max(self.end+down,0), min(self.end+up,genome_size[self.chrom]))
        else:
    ...
    def getintron(self):
        ...
        for nn in range(len(self.exon)-1):
            ...
            intron_end = self.exon[nn+1][0]
            assert intron_start < intron_end
            self.intron.append((intron_start,intron_end))
    def gettss(self):
        if self.strand == '+':
            self.tss = self.start
        else:
    ...
    def getdownstream(self,size,genome_size):
        key = str(size)
        if self.strand == '+':
            self.downstream[key] = (self.end, min(self.end+size,genome_size[self.chrom]))
        elif self.strand == '-':
...
def logging(text):
    print("Logging: " + text, file = sys.stderr)
...
def error(text,code=None):
    if code is not None:
        print("Error Num:%d %s" % (code,text), file = sys.stderr)
    else:
...
def load_region(filename):
    ...
    for line in fin:
        ...
        region_list.append(Region(chrom, start, end))
    fin.close()
    return region_list
...
def load_gene(bed_filename):
    ...
    for line in fin: 
        ...
        for nn in range(len(exon_start)):
            ...
            exon_end_pos = exon_start_pos + exon_len[nn]
            exon_list.append((exon_start_pos,exon_end_pos))
            if cds_start <= exon_end_pos and cds_end >= exon_start_pos:
                cds_start_pos = max(cds_start,exon_start_pos)
                cds_end_pos = min(cds_end,exon_end_pos)
                if cds_start_pos < cds_end_pos:
                    cds_list.append((cds_start_pos,cds_end_pos))
        genelist.append(Gene(chrom,start,end,strand,exon_list,cds_list,name,'NA'))
...
def merge_segment(filename):
    ...
    for line in fin:
        ...
        if tmp_start is None: # first line
            ...
            print("%s\t%d\t%d\t%s" % (tmp_chrom,tmp_start,tmp_end,tmp_group), file = tmpout)
            tmp_chrom = chrom
            tmp_start = start
            tmp_end = end
            tmp_group = group
            ...
            try:
                assert tmp_end == start # should be adjacent with each other
            except AssertionError:
                print(chrom, tmp_end, start, file = sys.stderr)
                print("Nearby regions are not adjacent. Is the bed file sorted.", file = sys.stderr)
                exit(1)
...
def get_seq_from_bed(chrom, start, end, genome):
    '''
    get seq from a genomic region
    --chrom     chromosome name
    --start     start position (0-base)
    --end      end position (0-base)
    --genome    pysam.Fastafile object
    '''
    if isinstance(genome, pysam.Fastafile):
        seq = str(genome.fetch(str(chrom), start, end)).upper()
    else:
        print("Unknown genome fasta class", file = sys.stderr)
        exit(1)
    return seq

def subtract_bed(chrom, start, end, exclude_tabix_list):
    ...
    for exclude_tabix in exclude_tabix_list:
        result = exclude_tabix.query(chrom, start, end)
        for overlap in result:
            o_chrom = overlap[0]
            o_start = int(overlap[1])
            o_end = int(overlap[2])
            exclude_start = max(o_start, start)
            exclude_end = min(o_end, end)
            remove[(exclude_start, exclude_end)] = True
            pos.append(exclude_start)
            pos.append(exclude_end)
        continue
        try:
            result = exclude_tabix.query(chrom, start, end)
            for overlap in result:
    ...
    for nn in range(len(pos)-1):
        ...
        if (start, end) in remove:
            ...
            if is_remove:
                ...
                region_list.append(BED(chrom, start, end))
    return region_list
...
def sort_bed(filename,is_cover):
    ...
    for line in fin:
        ...
        try:
            table[chrom].append([chrom,start,end,other])
        except KeyError:
            table[chrom] = [[chrom,start,end,other]]
    sorted_table = {}
    ...
    for chrom in sorted_chrom:
        for bed in sorted_table[chrom]:
            print(bed[0] + '\t' + str(bed[1]) + '\t' + str(bed[2]) + '\t' + bed[3], file = tmpout)
    tmpout.flush()
    fin.close()
    if is_cover:
        os.system("cp %s %s" % (tmpout.name,filename))
